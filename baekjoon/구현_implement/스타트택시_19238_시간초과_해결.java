...
public class 스타트택시_19238_시간초과_해결 {
    ...
    //알고리즘:구현, 최단거리니까 bfs
    //풀이1(틀렸음, 시간초과):
    //getDistance라는 메서드는 특정 지점을 지정해서 A에서 B까지의 거리를 BFS를 이용해 구했다.
    //즉, 현재 택시 위치(A) -> 손님 시작위치(B) 를 손님 수만큼 반복했다.
    //여기서 시간초과가 걸린것 같았다.
    //어처피 택시 위치는 고정되어 있어서, 현재 위치부터 모든 맵을 돌면서 손님의 시작위치일 경우에 거리만 저장하면 됬었다.
    //-> 즉 한번의 검색으로 모든 손님의 시작위치까지의 거리를 검색할 수 있다.

    //풀이2(맞았음):
    //위에서 시간초과가 왜 걸린지 유추한대로 getPassenger 메서드를 구현해서 맞았다.
    //getDistance는 손님을 태워서 시작->도착까지의 거리를 잴때 또 사용했다.
