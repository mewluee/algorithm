...
public class S2_가장긴증가하는부분수열_11053 {
    ...
    //예를 들어 n=1, m=0일때
    //dp[0]은 nums[0]=10 보다 작은 숫자이기 때문에, 10인 자기자신은 카운트에 포함되지 않는다.
    //결과적으로 dp[0]=0이 될것이다. 10보다 작은 숫자는 없어서 0개이기 때문이다.

    //그런데 dp[1]을 구한다고 하면,
    //dp[1](nums[1]=20보다 작은 개수) = dp[0](nums[0]=10보다 작은 숫자개수) + 1(nums[0]=10) 이 되는 것이다.
    //결과적으로 dp[1]=dp[0]+1 이 성립된다.
